...
# List of items
my_list = ['One', 'Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten', 'Eleven', 'Twelve', 'Thirteen', 'Fourteen', 'Fifteen', 'Sixteen', 'Seventeen', 'Eighteen', 'Nineteen', 'Twenty']
for item in my_list:
    my_listbox.insert(END, item)


def delete():  # Only works for SINGLE
    my_listbox.delete(ANCHOR)
    my_label.config(text='')

def select():  # Only works for SINGLE
...
def select_all():
    result = ''
    for item in my_listbox.curselection():  # Gives list number of selection
